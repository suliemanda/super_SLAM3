...
public:
  ...
  bool infer(const cv::Mat &image, const cv::Mat &mask,
             std::vector<cv::KeyPoint> &keypoints, cv::OutputArray &descriptor,std::vector<float> &scores);
...
private:
  ...
  bool process_output(const tensorrt_buffer::BufferManager &buffers,
                      std::vector<cv::KeyPoint> &keypoints, cv::OutputArray &descriptors,std::vector<float> &scores,
                      const cv::Mat &mask);
