...
bool SuperPoint::infer(const cv::Mat &image, const cv::Mat &mask,std::vector<cv::KeyPoint> &keypoints, cv::OutputArray &descriptors,std::vector<float> &scores)
{
  ...
  if (!process_output(buffers, keypoints, descriptors,scores, mask))
  {
    return false;
  }
  return true;
...
bool SuperPoint::process_output(
    ...
    cv::OutputArray &descriptors,std::vector<float> &scores, const cv::Mat &mask)
{
  keypoints_.clear();
  descriptors_.clear();
...
for (const auto& keypoint : keypoints_) {
    keypoints.emplace_back(keypoint[0], keypoint[1], 1.0f);
}
                     
  // keypoints = keypoints_;
  descriptors.create( keypoints_.size(), 256,CV_32F);
  scores.clear();
  scores.resize(scores_vec.size());
  cv::Mat descriptors_mat = descriptors.getMat();
  for (int i = 0; i < keypoints_.size(); i++) {
  scores[i] = scores_vec[i];
  for (int j = 0; j < 256; j++) {
  descriptors_mat.at<float>(i, j) = descriptors_[i][j];
