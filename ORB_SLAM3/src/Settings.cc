...
#include "CameraModels/Equirectangular.h"
    ...
    void Settings::readCamera1(cv::FileStorage &fSettings) {
        ...
        if (cameraModel == "PinHole") {
            ...
            if(sensor_ == System::STEREO || sensor_ == System::IMU_STEREO){
                ...
                else if(cameraModel == "Equirectangular"){
            cameraType_ = EQ;

            //Read intrinsic parameters
            float col = readParameter<float>(fSettings,"Camera1.col",found);
            float row = readParameter<float>(fSettings,"Camera1.row",found);
            

            vCalibration = {col,row};

            calibration1_ = new Equirectangular(vCalibration);
            originalCalib1_ = new Equirectangular(vCalibration);
            ...
            // if(sensor_ == System::STEREO || sensor_ == System::IMU_STEREO){
                ...
                // vector<int> vOverlapping = {colBegin, colEnd};
// 
                // static_cast<Equirectangular*>(calibration1_)->mvLappingArea = vOverlapping;
            // }
        }
        else{
    ...
    ostream &operator<<(std::ostream& output, const Settings& settings){
        ...
        if(settings.cameraType_ == Settings::EQ)
        {
